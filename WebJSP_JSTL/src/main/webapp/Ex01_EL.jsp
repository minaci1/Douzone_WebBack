...
EL(출력 표현식) : JSP 패이지에서 사용되는 출력 전용 스크립트 언어 (화면..) POINT) 서버에서 해석되는 스크립트 언어
이유 : 스파게티 코드 % + UI(정적) > 유지보수 어려움 > 코드의 가독성(해석)
문제) EL & JSTL 만들어서 html 과 잘 놀고 .. JSP 사용가능 

EL : tomcat 서버가 내장하고 있는 자원 (별도의 참조 필요없다.)
JSTL : 다운로드해서 (빌드 path ..) WEB-INF > lib > 복사 붙여넣기 > jar 파일
